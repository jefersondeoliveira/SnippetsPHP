...
/*
 * classe TRecord
 *  Esta classe provê os métodos necessários para persitir e
...
abstract class TRecord
{
	...
	public function store()
	{
		...
		else
		{
			...
			foreach ($this->data as $key => $value)
			{
				if($key !== 'id') // o ID não precia ir no UPDATE
				{
					//passa os dados do objeto para o SQL
					$sql->setRowData($key, $this->$key);
				}
		...
		{
			//se não tiver transaçãom, retorna uma exceção
	...
	public function delete($id = NULL)
	{
		...
		$criteria = new TCriteria;
		$criteria->add(new TFilter(' id ', '= ', $id));
		//define o critério de seleção baseada no ID
		$sql->setCriteria($criteria);
